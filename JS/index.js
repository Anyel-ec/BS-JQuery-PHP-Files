...
function validarMensaje(mensaje) {
    var palabrasObscenas = ["puto", "vrg", "verga", 'tonto', 'animal', 'ladrones', 'vaga', 
    'vago', 'brutica', 'feo', "cv", 'hp']; // Lista de palabras obscenas
    
    for (var i = 0; i < palabrasObscenas.length; i++) {
        if (mensaje.toLowerCase().indexOf(palabrasObscenas[i]) !== -1) {
            return false; // Mensaje contiene una palabra obscena
        }
    }
    
    return true; // Mensaje es aceptable
}
...
function validarFormulario() {
    ...
    // Eliminar espacios en blanco del nombre, apellido, correo y mensaje
    nombre = nombre.trim();
    apellido = apellido.trim();
    correo = correo.trim();
    mensaje = mensaje.trim();
    
    // Verificar que el nombre y el apellido contengan solo letras y espacios (incluyendo la letra "ñ")
    if (!/^[A-Za-zÑñ\s]+$/.test(nombre) || !/^[A-Za-zÑñ\s]+$/.test(apellido)) {
    ...
    if (!/^\d{10}$/.test(celular)) {
        alert("El número de celular debe tener exactamente 10 dígitos y ser numérico.");
        return false;
    }
    ...
    // Validar el mensaje contra palabras obscenas
    if (!validarMensaje(mensaje)) {
        alert("El mensaje contiene contenido obsceno o inapropiado.");
        return false;
    }
